...
def prepare_rag_context(question: str) -> dict:
    """Prepares retrieval context, prompt templates, and diagnostics for RAG pipelines."""
    import time
    import re
...
def parse_rag_response(answer: str, fallback_sources: list, fallback_repos: list) -> dict:
    """Parses structured output from LLM answer and calculates confidence score."""
    confidence = 0.9
...
def run_hybrid_rag(question: str) -> dict:
    ...
    ctx = prepare_rag_context(question)

    d = ctx["diagnostics"]
    repo_filter_str = str(d["repo_filter"]) if d["repo_filter"] else "None"
    print("========================================")
    print("QUERY CLASS")
    print(d["query_class"])
    ...
    print(d["final_chunks_count"])
    print("Context Characters")
    print(d["total_chars"])
    print("Estimated Tokens")
    print(d["est_prompt_tokens"])
    print("========================================")

    if not ctx["merged_context"].strip():
...
def run_hybrid_rag_stream(question: str):
    ...
    full_response = ""
    try:
        for chunk in stream_llm_with_retry(ctx["messages"]):
            full_response += chunk
            yield {
                "type": "token",
                "content": chunk
    ...
    result = parse_rag_response(full_response, ctx["sources"], ctx["repos"])
    total_duration = time.perf_counter() - ctx["start_time"]
